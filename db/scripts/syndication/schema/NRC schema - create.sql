...
BEGIN
	IF EXISTS(
	    SELECT schema_name
	      ...
	      WHERE schema_name = 'nrc'
	  )
	THEN
	  DROP SCHEMA nrc CASCADE;
	END IF;
END
$$;

CREATE SCHEMA nrc AUTHORIZATION cloudsqlsuperuser;

-- Drop table

-- DROP TABLE nrc.countries;

-- NOTE: don't copy over country data to public, map it instead

CREATE TABLE nrc.countries (
...
-- DROP TABLE nrc.settlements;

CREATE TABLE nrc.settlements (
	id bigserial NOT NULL,
	"name" varchar NULL,
...
ALTER TABLE nrc.settlements ADD CONSTRAINT settlements_country_id_foreign FOREIGN KEY (country_id) REFERENCES nrc.countries(id);

-- Drop table

-- DROP TABLE nrc.service_types;

-- NOTE: don't copy over service type data to public, map it instead

CREATE TABLE nrc.service_types (
	id bigserial NOT NULL,
	"name" varchar NULL,
	created_at timestamptz NOT NULL DEFAULT now(),
	updated_at timestamptz NOT NULL DEFAULT now(),
...
-- DROP TABLE nrc.service_points;

CREATE TABLE nrc.service_points (
	id bigserial NOT NULL,
...
ALTER TABLE nrc.service_points ADD CONSTRAINT service_points_service_type_id_foreign FOREIGN KEY (service_type_id) REFERENCES nrc.service_types(id);
ALTER TABLE nrc.service_points ADD CONSTRAINT service_points_settlement_id_foreign FOREIGN KEY (settlement_id) REFERENCES nrc.settlements(id);


-- Drop table

-- DROP TABLE nrc.users;

CREATE TABLE nrc.users (
	id bigserial NOT NULL,
...
ALTER TABLE nrc.users ADD CONSTRAINT users_settlement_id_foreign FOREIGN KEY (settlement_id) REFERENCES nrc.settlements(id);

-- Drop table

-- DROP TABLE nrc.responses;

CREATE TABLE nrc.responses (
	id bigserial NOT NULL,
...
ALTER TABLE nrc.responses ADD CONSTRAINT responses_service_point_id_foreign FOREIGN KEY (service_point_id) REFERENCES nrc.service_points(id);
ALTER TABLE nrc.responses ADD CONSTRAINT responses_user_id_foreign FOREIGN KEY (user_id) REFERENCES nrc.users(id);
