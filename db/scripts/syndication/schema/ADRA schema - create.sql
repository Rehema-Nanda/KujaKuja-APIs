...
BEGIN
	IF EXISTS(
	    SELECT schema_name
	      ...
	      WHERE schema_name = 'adra'
	  )
	THEN
	  DROP SCHEMA adra CASCADE;
	END IF;
END
$$;

CREATE SCHEMA adra AUTHORIZATION cloudsqlsuperuser;

-- Drop table

-- DROP TABLE adra.countries;

-- NOTE: don't copy over country data to public, map it instead

CREATE TABLE adra.countries (
...
-- DROP TABLE adra.settlements;

CREATE TABLE adra.settlements (
	id bigserial NOT NULL,
	"name" varchar NULL,
...
ALTER TABLE adra.settlements ADD CONSTRAINT settlements_country_id_foreign FOREIGN KEY (country_id) REFERENCES adra.countries(id);

-- Drop table

-- DROP TABLE adra.service_types;

-- NOTE: don't copy over service type data to public, map it instead

CREATE TABLE adra.service_types (
	id bigserial NOT NULL,
	"name" varchar NULL,
	created_at timestamptz NOT NULL DEFAULT now(),
	updated_at timestamptz NOT NULL DEFAULT now(),
...
-- DROP TABLE adra.service_points;

CREATE TABLE adra.service_points (
	id bigserial NOT NULL,
...
ALTER TABLE adra.service_points ADD CONSTRAINT service_points_service_type_id_foreign FOREIGN KEY (service_type_id) REFERENCES adra.service_types(id);
ALTER TABLE adra.service_points ADD CONSTRAINT service_points_settlement_id_foreign FOREIGN KEY (settlement_id) REFERENCES adra.settlements(id);


-- Drop table

-- DROP TABLE adra.users;

CREATE TABLE adra.users (
	id bigserial NOT NULL,
...
ALTER TABLE adra.users ADD CONSTRAINT users_settlement_id_foreign FOREIGN KEY (settlement_id) REFERENCES adra.settlements(id);

-- Drop table

-- DROP TABLE adra.responses;

CREATE TABLE adra.responses (
	id bigserial NOT NULL,
...
ALTER TABLE adra.responses ADD CONSTRAINT responses_service_point_id_foreign FOREIGN KEY (service_point_id) REFERENCES adra.service_points(id);
ALTER TABLE adra.responses ADD CONSTRAINT responses_user_id_foreign FOREIGN KEY (user_id) REFERENCES adra.users(id);
