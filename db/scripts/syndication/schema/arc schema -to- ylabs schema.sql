...
left join (VALUES('Uganda', 124, 1)) AS arc_ylabs_country_map (service_type_name, src_country_id, target_country_id)
on s.country_id = arc_ylabs_country_map.src_country_id
WHERE id < 1000000000 and id in (4);

-- hack!! using settlement id 4 to bring users across.
...
is_admin, provider, concat(uid,:email_suffix), tokens, 4 + :offset_start, is_survey, is_service_provider
...
left join (VALUES('Community Pulse', 7, 1)) AS arc_ylabs_sp_map (service_type_name, src_id, target_service_type_id)
on sp.service_type_id=arc_ylabs_sp_map.src_id WHERE id < 1000000000 and id in (632); -- this is co-incidentally the same as the settlement id
...
FROM :schema_name.responses WHERE id < 1000000000 and service_point_id in (632) AND created_at > '1 Jan 2020';
