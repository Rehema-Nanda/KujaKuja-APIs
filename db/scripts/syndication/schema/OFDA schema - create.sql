...
BEGIN
	IF EXISTS(
	    SELECT schema_name
	      ...
	      WHERE schema_name = 'ofda'
	  )
	THEN
	  DROP SCHEMA ofda CASCADE;
	END IF;
END
$$;

CREATE SCHEMA ofda AUTHORIZATION cloudsqlsuperuser;

-- Drop table

-- DROP TABLE ofda.countries;

-- NOTE: don't copy over country data to public, map it instead

CREATE TABLE ofda.countries (
...
-- DROP TABLE ofda.settlements;

CREATE TABLE ofda.settlements (
	id bigserial NOT NULL,
	"name" varchar NULL,
...
ALTER TABLE ofda.settlements ADD CONSTRAINT settlements_country_id_foreign FOREIGN KEY (country_id) REFERENCES ofda.countries(id);

-- Drop table

-- DROP TABLE ofda.service_types;

-- NOTE: don't copy over service type data to public, map it instead

CREATE TABLE ofda.service_types (
	id bigserial NOT NULL,
	"name" varchar NULL,
	created_at timestamptz NOT NULL DEFAULT now(),
	updated_at timestamptz NOT NULL DEFAULT now(),
...
-- DROP TABLE ofda.service_points;

CREATE TABLE ofda.service_points (
	id bigserial NOT NULL,
...
ALTER TABLE ofda.service_points ADD CONSTRAINT service_points_service_type_id_foreign FOREIGN KEY (service_type_id) REFERENCES ofda.service_types(id);
ALTER TABLE ofda.service_points ADD CONSTRAINT service_points_settlement_id_foreign FOREIGN KEY (settlement_id) REFERENCES ofda.settlements(id);


-- Drop table

-- DROP TABLE ofda.users;

CREATE TABLE ofda.users (
	id bigserial NOT NULL,
...
ALTER TABLE ofda.users ADD CONSTRAINT users_settlement_id_foreign FOREIGN KEY (settlement_id) REFERENCES ofda.settlements(id);

-- Drop table

-- DROP TABLE ofda.responses;

CREATE TABLE ofda.responses (
	id bigserial NOT NULL,
...
ALTER TABLE ofda.responses ADD CONSTRAINT responses_service_point_id_foreign FOREIGN KEY (service_point_id) REFERENCES ofda.service_points(id);
ALTER TABLE ofda.responses ADD CONSTRAINT responses_user_id_foreign FOREIGN KEY (user_id) REFERENCES ofda.users(id);
