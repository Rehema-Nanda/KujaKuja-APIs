...
BEGIN
	...
	THEN
	  DROP SCHEMA chatbot CASCADE;
	END IF;
END
$$;

CREATE SCHEMA chatbot AUTHORIZATION cloudsqlsuperuser;

-- Drop table

-- DROP TABLE chatbot.countries;

-- NOTE: don't copy over country data to public, map it instead

CREATE TABLE chatbot.countries (
...
-- DROP TABLE chatbot.settlements;

CREATE TABLE chatbot.settlements (
	id bigserial NOT NULL,
	"name" varchar NULL,
...
ALTER TABLE chatbot.settlements ADD CONSTRAINT settlements_country_id_foreign FOREIGN KEY (country_id) REFERENCES chatbot.countries(id);

-- Drop table

-- DROP TABLE chatbot.service_types;

-- NOTE: don't copy over service type data to public, map it instead

CREATE TABLE chatbot.service_types (
	id bigserial NOT NULL,
	"name" varchar NULL,
	created_at timestamptz NOT NULL DEFAULT now(),
	updated_at timestamptz NOT NULL DEFAULT now(),
...
-- DROP TABLE chatbot.service_points;

CREATE TABLE chatbot.service_points (
	id bigserial NOT NULL,
...
ALTER TABLE chatbot.service_points ADD CONSTRAINT service_points_service_type_id_foreign FOREIGN KEY (service_type_id) REFERENCES chatbot.service_types(id);
ALTER TABLE chatbot.service_points ADD CONSTRAINT service_points_settlement_id_foreign FOREIGN KEY (settlement_id) REFERENCES chatbot.settlements(id);
...
ALTER TABLE chatbot.users ADD CONSTRAINT users_settlement_id_foreign FOREIGN KEY (settlement_id) REFERENCES chatbot.settlements(id);

-- Drop table

-- DROP TABLE chatbot.responses;
...
ALTER TABLE chatbot.responses ADD CONSTRAINT responses_service_point_id_foreign FOREIGN KEY (service_point_id) REFERENCES chatbot.service_points(id);
ALTER TABLE chatbot.responses ADD CONSTRAINT responses_user_id_foreign FOREIGN KEY (user_id) REFERENCES chatbot.users(id);
