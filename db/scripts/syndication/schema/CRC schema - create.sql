...
BEGIN
	IF EXISTS(
	    SELECT schema_name
	      ...
	      WHERE schema_name = 'crc'
	  )
	THEN
	  DROP SCHEMA crc CASCADE;
	END IF;
END
$$;

CREATE SCHEMA crc AUTHORIZATION cloudsqlsuperuser;

-- Drop table

-- DROP TABLE crc.countries;

-- NOTE: don't copy over country data to public, map it instead

CREATE TABLE crc.countries (
...
-- DROP TABLE crc.settlements;

CREATE TABLE crc.settlements (
	id bigserial NOT NULL,
	"name" varchar NULL,
...
ALTER TABLE crc.settlements ADD CONSTRAINT settlements_country_id_foreign FOREIGN KEY (country_id) REFERENCES crc.countries(id);

-- Drop table

-- DROP TABLE crc.service_types;

-- NOTE: don't copy over service type data to public, map it instead

CREATE TABLE crc.service_types (
	id bigserial NOT NULL,
	"name" varchar NULL,
	created_at timestamptz NOT NULL DEFAULT now(),
	updated_at timestamptz NOT NULL DEFAULT now(),
...
-- DROP TABLE crc.service_points;

CREATE TABLE crc.service_points (
	id bigserial NOT NULL,
...
ALTER TABLE crc.service_points ADD CONSTRAINT service_points_service_type_id_foreign FOREIGN KEY (service_type_id) REFERENCES crc.service_types(id);
ALTER TABLE crc.service_points ADD CONSTRAINT service_points_settlement_id_foreign FOREIGN KEY (settlement_id) REFERENCES crc.settlements(id);


-- Drop table

-- DROP TABLE crc.users;

CREATE TABLE crc.users (
	id bigserial NOT NULL,
...
ALTER TABLE crc.users ADD CONSTRAINT users_settlement_id_foreign FOREIGN KEY (settlement_id) REFERENCES crc.settlements(id);

-- Drop table

-- DROP TABLE crc.responses;

CREATE TABLE crc.responses (
	id bigserial NOT NULL,
...
ALTER TABLE crc.responses ADD CONSTRAINT responses_service_point_id_foreign FOREIGN KEY (service_point_id) REFERENCES crc.service_points(id);
ALTER TABLE crc.responses ADD CONSTRAINT responses_user_id_foreign FOREIGN KEY (user_id) REFERENCES crc.users(id);
